...
func buildResult(edited *core.Media, base *doc, newItems []item, lay layout, delta, total, newIlstLen int64) *core.Media {
	...
	// A Nero chpl is a udta sibling of meta: it does not change here, but it shifts when
	// it sits after the rewritten ilst region.
...
// shiftStructure relocates the parts of a rewritten document that move when a metadata
// region changes size: it grows the moov box and shifts, by delta, every chunk-offset
// and sample-auxiliary table entry past the insertion point plus any offset-table atom,
// mdat range, and top-level atom that lies at or past regionEnd.
...
// shiftTables relocates one group of offset tables into a result document: the atom's
// own offset moves when it lies at or past regionEnd, and every entry past the
// insertion point moves by delta.
...
// resultUdtaRaw reconstructs the post-write udta payload after a tag/picture edit by
// applying the same ilst-region change to the captured udta bytes, so a chapter edit on
// the returned document can splice into them.
func resultUdtaRaw(base *doc, lay layout) []byte {
	if base.udta != nil && base.udtaRaw != nil {
		...
		// The splice resized the ilst region inside the enclosing meta box, so meta's own
		// size field in these bytes is stale. Bounds checks keep malformed layouts from
