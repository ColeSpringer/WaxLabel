...
func FuzzParse(f *testing.F) {
	...
	f.Add([]byte("\x00\x00\x00\x10ftypM4A \x00\x00\x00\x08moof"))                                                                                         // ftyp(16) swallows the trailing bytes: no moov, must reject not panic
	f.Add([]byte("\x00\x00\x00\x10ftypM4A \x00\x00\x00\x00\x00\x00\x00\x08moov\x00\x00\x00\x08moof\x00\x00\x00\x10mdat\x00\x00\x00\x00\x00\x00\x00\x00")) // fragmented: parses, reaches Plan, must refuse not panic
	f.Add([]byte("\x00\x00\x00\x10ftypM4A \x00\x00\x00\x00\x00\x00\x00\x08styp\x00\x00\x00\x08moof"))                                                     // fragmented segment (no moov): must reject, not panic
...
func FuzzChainedMP4ChapterWrite(f *testing.F) {
	...
	accept := func(err error) bool {
		return errors.Is(err, waxerr.ErrInvalidData) || errors.Is(err, waxerr.ErrSizeTooLarge) ||
			...
			errors.Is(err, waxerr.ErrFragmented)
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		doc, err := wl.Parse(ctx, wl.BytesSource(data))
