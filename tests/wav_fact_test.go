...
// MS-ADPCM case: avgBytesPerSec is nominal (16000 against ~22 KB of one second's audio), so the
// byte-rate formula reads 1.4 s and the declared 128 kbps is not what the file holds.
...
// sanity gate: dwSampleLength is attacker-controlled, so a 0xFFFFFFFF count on a 22 KB file must
// not report a 27-hour duration. The byte-rate estimate takes over instead.
...
// with no byte rate to compare against, a declared count and a declared rate can be nonsense
// together and stay inside every ratio. 800000 frames at 1 Hz is 9.3 days of audio stored in 100
// KB, which is 1 bit per second; the bitrate floor is what sees it.
...
// without a fact chunk, blockAlign is a compressed block, so reporting dataLen/blockAlign would be
// a block count three orders of magnitude off. The duration still falls back to the nominal byte
// rate.
...
// for a constant-rate family the fmt byte rate is exact, so the declared count changes nothing. A
// fact chunk in a PCM file must not move the numbers.
...
// fact chunk is preserved verbatim like every other non-tag chunk, so the count still describes the
// audio after a metadata rewrite.
